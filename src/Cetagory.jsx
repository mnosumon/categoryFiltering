...
const Cetagory = ({categories, fitlerItem, setItems}) => {
  ...
  return (
    <>
        <div className="flex gap-5 my-10 justify-center items-center">
            {
                categories.map(item=>(
                    <button className='text-3xl capitalize bg-orange-500 py-4 px-8 rounded-xl' onClick={() => fitlerItem(item)}>{item}</button>
                ))
            }
